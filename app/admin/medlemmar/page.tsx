  ...
  return (
    <div className="max-w-4xl mx-auto bg-card-white text-gray-800 rounded-xl shadow-2xl border border-gray-200/10">
      <div className="p-4 md:p-8">
        <h1 className="text-3xl md:text-5xl font-serif font-bold text-gray-200 mb-2">Hantera Medlemmar</h1>
        <p className="text-gray-300 mb-6 md:mb-8">Lägg till, redigera, arkivera eller ta bort medlemmar i systemet.</p>
        
        {/* Sektion: Lägg till ny medlem */}
        <div className="border-t border-gray-200 pt-6">
          <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-200">Lägg till ny medlem</h2>
          <form onSubmit={handleAddMember} className="space-y-4">
            <label htmlFor="newName" className="block text-sm font-medium text-gray-300">Namn & Roll</label>
            <div className="flex flex-col sm:flex-row gap-3">
              <input type="text" id="newName" value={newName} onChange={e => setNewName(e.target.value)}
                ...
                placeholder="Medlemmens namn..." required />
        ...
        <div className="mt-10">
          <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-200">Aktiva Medlemmar</h2>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <ul className="divide-y divide-gray-200">
              {activeMembers.map(member => (
                <li key={member.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 gap-3 bg-gray-600/30 hover:bg-gray-600/50 transition-colors">
                  {editingMemberId === member.id ? (
                    // Redigeringsläge
                    <div className="flex-grow flex flex-col sm:flex-row items-stretch gap-2 w-full">
                      <input type="text" value={editingName} onChange={(e) => setEditingName(e.target.value)} className="form-input flex-grow bg-white border border-gray-500/40 text-gray-100 rounded-md p-1 w-full"/>
                  ...
                  <div className="flex items-center justify-end space-x-2 flex-shrink-0">
                    {editingMemberId === member.id ? (
                      <>
                        <button onClick={() => handleSaveClick(member.id)} className="bg-green-600 text-white text-xs font-bold py-1 px-3 rounded-md hover:bg-green-700">Spara</button>
                      ...
                      <>
                        <button onClick={() => handleEditClick(member)} className="font-semibold text-xs py-1 px-3 rounded-md text-gray-200 hover:bg-gray-600">Redigera</button>
                        <button onClick={() => handleArchiveMember(member.id, member.name)} className="font-semibold text-xs py-1 px-3 rounded-md text-gray-200 hover:bg-gray-600">Arkivera</button>
                      </>
                    )}
                  </div>
                </li>
        ...
        <div className="mt-12 border-t border-gray-200 pt-8">
            <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-200">Arkiverade Medlemmar</h2>
            <p className="text-sm text-gray-300 mb-4">Medlemmar här är dolda från vanliga listor men deras historik finns kvar. Härifrån kan de återaktiveras eller raderas permanent.</p>
            <div className="border border-gray-300 rounded-lg overflow-hidden">
                <ul className="divide-y divide-gray-200">
                    {archivedMembers.map(member => (
                        <li key={member.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 gap-2 bg-gray-600/50">
                            <span className="p-1 font-medium text-gray-50 italic mb-2 sm:mb-0">{member.name}</span>
                            <div className="flex items-center justify-end space-x-2 flex-shrink-0">
                                <button onClick={() => handleReactivateMember(member.id, member.name)} className="font-semibold text-xs py-1 px-3 rounded-md text-green-500 hover:bg-green-100">Återställ</button>
