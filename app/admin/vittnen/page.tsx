...
export default function AdminWitnessesPage() {
  ...
  const handleAddWitness = async (e: FormEvent) => {
    ...
    setWitnesses((prev) =>
      [newWitnessOptimistic, ...prev].sort((a, b) =>
  ...
  const handleDeleteWitness = async (id: string, name: string) => {
    if (!window.confirm(`Är du säker på att du vill ta bort vittnet ${name}?`))
      return;
    ...
    <div className="max-w-4xl mx-auto bg-card-white text-gray-800 rounded-xl shadow-2xl border border-gray-300">
      <div className="p-4 md:p-8">
        <h1 className="text-3xl md:text-5xl font-serif font-bold text-gray-200 mb-2">
          Hantera Externa Vittnen
        </h1>
        <p className="text-gray-300 mb-6 md:mb-8">
          Lägg till, redigera eller ta bort personer i externa vittneslistan.
        </p>

        {/* Sektion för att lägga till nytt vittne */}
        <div className="border-t border-gray-200 pt-6">
          <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-300">
            Lägg till nytt externt vittne
          </h2>
          <form onSubmit={handleAddWitness}>
            ...
            <div className="mt-1 flex flex-col sm:flex-row gap-3">
              <input
                ...
                placeholder="Vittnets namn..."
                required
              />
              <button
                type="submit"
                className="w-full sm:w-auto flex-shrink-0 bg-green-600 text-white font-bold py-2 px-6 rounded-md hover:bg-green-700 transition-colors"
              >
                Lägg till
              </button>
            </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <ul className="divide-y divide-gray-200">
              {witnesses.map((witness) => (
                <li
                  key={witness.id}
                  className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 gap-3 text-gray-100 bg-gray-600/30 hover:bg-gray-600/50"
                >
                  <div className="flex-grow mb-2 sm:mb-0">
                    {editingWitnessId === witness.id ? (
                      <input
                        ...
                        className="form-input w-full bg-white border border-gray-400 rounded-md p-1"
                        autoFocus
                      />
                    ) : (
                  ...
                  <div className="flex items-center justify-end space-x-2 flex-shrink-0">
                    {editingWitnessId === witness.id ? (
                      <>
                        <button
                          onClick={() => handleSaveClick(witness.id)}
                      ...
                      <>
                        ...
                        <button
                          onClick={() =>
                            handleDeleteWitness(witness.id, witness.name)
                          }
                          className="text-white bg-red-600 font-semibold text-xs py-1 px-3 rounded-md hover:bg-red-700"
                        >
                          Ta bort
                        </button>
                      </>
