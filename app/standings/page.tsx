...
function MemberCard({ member, index }: { member: Member; index: number }) {
  ...
  const colorClass =
    suit === "♥" || suit === "♦" ? "text-red-500" : "text-gray-200";

  const getCardValue = (r: number): string => {
    const effectiveRank = ((r - 1) % 13) + 1;

    if (effectiveRank === 1) return "A";
    if (effectiveRank >= 2 && effectiveRank <= 10)
      return effectiveRank.toString();
  ...
  return (
    <Link href={`/person/${member.id}`} className="card-container">
      <div className="member-card bg-card-white text-gray-600 rounded-xl p-4 flex flex-col justify-between h-full card-inner-border">
        ...
        <div
          className={`absolute inset-0 flex items-center justify-center text-[12rem] ${colorClass} opacity-10 font-serif select-none`}
        >
          {suit}
        </div>
        ...
        {/* Övre vänstra hörnet */}
        <div
          className={`absolute top-4 left-4 text-center leading-none font-bold text-2xl ${colorClass}`}
        >
          <div>{cardValue}</div>
          <div>{suit}</div>
        </div>
        ...
        {/* Nedre högra hörnet */}
        <div
          className={`absolute bottom-4 right-4 text-center leading-none font-bold text-2xl ${colorClass} transform rotate-180`}
        >
          <div>{cardValue}</div>
          <div>{suit}</div>
        </div>
    ...
    <div>
      ...
      {members.length === 0 ? (
        <div className="text-center text-gray-400 bg-gray-900/50 p-8 rounded-lg">
          ...
          <p className="mt-2">
            Gå till adminpanelen för att lägga till den första medlemmen.
          </p>
        </div>
      ) : (
        <>
          ...
          {kexMembers.length > 0 && (
            <section className="mb-16">
              <h2 className="text-4xl font-serif font-semibold text-center mb-6 text-gray-300 border-b-2 border-amber-400 pb-2 mt-6">
                KEX
              </h2>
          ...
          {essMembers.length > 0 && (
            <section>
              <h2 className="text-4xl font-serif font-semibold text-center mb-6 text-gray-300 border-b-2 border-red-400 pb-2 mt-6">
                ESS
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {jokerMembers.length > 0 && (
            <section>
              <h2 className="text-4xl font-serif font-semibold text-center mb-6 text-gray-300 border-b-2 border-purple-400 pb-2 mt-6">
                JOKER
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
